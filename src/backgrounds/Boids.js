...
const Boids = ({flockSize, separVal, alignVal, cohesVal, windVal, bounds, spawnRange}) => {
    ...
    for (let i = 0; i < flockSize; i++) {
        ...
        flock.push(
            <Bird
                ...
                ref={ref}
                scale={[9, 9, 9]}
                position={[Math.random() * spawnRange * 2 - spawnRange, Math.random() * spawnRange * 2 - spawnRange, Math.random() * spawnRange * 2 - spawnRange]}
...
const Boundary = ({ bounds }) => {
    return <mesh>
        <boxBufferGeometry args={[bounds, bounds, bounds, 1, 1]} attach='geometry' />
        <meshStandardMaterial transparent side={DoubleSide} opacity={0.1} color='white' attach='material' />
    </mesh>
}

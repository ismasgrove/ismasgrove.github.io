...
const SceneMenu = (props) => {
    ...
    const [scrollable, setScrollable] = React.useState({left: false, right: true})
    ...
    const onScroll = () => {
        ...
        if (ref.current.clientWidth == ref.current.scrollWidth - ref.current.scrollLeft) {
            right = false
        }
        else {
            right = true
    ...
    return (
        <div ref={ref}  {...props}
            ...
            onScroll={onScroll}
        >
            {Object.keys(scenes).map((sceneObj) => <div key={sceneObj} tw='min-w-max self-center cursor-pointer' onClick={() => {
                if (sceneObj != scene) {
                    navigate(scenes[sceneObj])
                }
            }
            }>
                <img tw='w-[16rem] h-[8rem] self-center object-fill border-[1px] border-[var(--scrollable-color)]'
                    src={`/${sceneObj.toLowerCase()}.jpg`} alt={sceneObj.toLowerCase()} />
            </div>
            )}
            </div>
    )
...
export default function SceneSelector(props) {
    return (
            <SceneMenu tw='prose font-mono z-10 bg-primary p-4' {...props} />
    )
}